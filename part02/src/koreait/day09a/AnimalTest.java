...
public class AnimalTest {
	...
	public static void main(String[] args) {
		...
		temp.print();
//		temp.run();					//오류 : 부모 타입 변수로는 자식 메소드 실행 못합니다.
		temp = crow;				//자식 객체는 부모 클래스 타입으로 참조할 수 있습니다.
		temp.print();
//		temp.fly();					//오류 : 부모 타입 변수로는 자식 메소드 실행 못합니다.
		System.out.println("-------------------------------");
		Animal puppy = new Dog();	//자식 객체는 부모 클래스 타입으로 참조할 수 있습니다.
		puppy.print();
//		puppy.run();				//오류 : 부모 타입 변수로는 자식 메소드 실행 못합니다.
