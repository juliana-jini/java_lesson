...
public class C34_StringArray {
	...
	public static void main(String[] args) {
		...
		//										 ㄴ 기본형 데이터 타입
		//engKey뿐만 아니라 engKey[2]도 String 참조형 타입
		
		for(int i=0;i<engKey.length;i++)
			System.out.println(engKey[i]);	//engKey[i]는 String 참조형
		
		String[] koreans = new String[5];	//배열 요소의 값이 기본형 데이터 저장 X, 참조값 저장
...
//		System.out.println(temp.length());	//오류
		...
		//사용자가 입력한 영어 단어가 engKey 배열 인덱스 몇 번인지?
		 ...
		 //break로 for를 중단하지 않고 for를 모두 실행했을 때는 같은 문자열이 배열 engKey에 없을 때 입니다.
		 //i=?
