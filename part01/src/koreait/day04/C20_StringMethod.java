...
public class C20_StringMethod {
	...
	public static void main(String[] args) {
		...
		//substring에서 지정할 수 있는 숫자는 0부터 ~ 6(length())까지 그 외의 값을 쓰면 IndexOutOfBounds 오류 발생합니다.
		
		System.out.println("substring(1, 4) = " + message.substring(1, 4));				// ell, 1번부터 3개(4-1)
		
		System.out.println("indexOf(\"ll\", \"*@\") = " + message.replace("ll", "*@"));	// 치환 : he*@o~
		System.out.println("startsWith(\"H\")" + message.startsWith("H"));				// false
		System.out.println("startsWith(\"~\")" + message.endsWith("~"));				// true
		
	}
	// "를 출력할 때는 \(역슬래쉬) 필요합니다. -> " 기호는 문자열 시작으로 해석되기 때문입니다.
}
